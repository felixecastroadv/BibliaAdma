...
export default function App() {
  ...
  // --- FUNÇÃO CRÍTICA DE RECUPERAÇÃO DE CONTA (FUZZY MATCHING & MERGE) ---
  const loadProgress = async (email: string, nameFallback?: string) => {
    try {
        ...
        // Isso evita problemas de case sensitivity e acentuação no backend e usa o cache local mergeado
        ...
        if (bestProfile) {
            ...
            if (bestProfile.total_chapters > 0) {
               // showToast(`Dados recuperados: ${bestProfile.total_chapters} capítulos lidos.`, "success"); // Opcional para não spammar
            }
        } else {
