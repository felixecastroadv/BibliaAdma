...
import { ChevronLeft, Settings, Type, Play, Pause, CheckCircle, Sparkles, FastForward, ChevronRight, List, Book, ChevronDown } from 'lucide-react';
...
// Skeleton Component para carregamento
const TextSkeleton = () => (
  <div className="space-y-6 animate-pulse mt-4">
    {[1, 2, 3, 4, 5, 6].map((i) => (
      <div key={i} className="flex gap-4">
        <div className="h-4 w-4 bg-gray-200 dark:bg-gray-700 rounded mt-2 shrink-0"></div>
...
export default function BibleReader({ userProgress, isAdmin, onProgressUpdate, onShowToast, onBack, initialBook, initialChapter }: any) {
  ...
  const fetchChapter = async () => {
    ...
    try {
        ...
        // CRÍTICO: Para livros curtos ou para garantir a integridade, pedimos explicitamente versículos 1 a 300.
        // A API bible-api.com ignora versículos que não existem, retornando até o último disponível.
        // Isso resolve o problema de Obadias retornar só verso 1.
        let queryUrl = `https://bible-api.com/${engBook}+${chapter}?translation=almeida`;
        
        // Forçar range para garantir que venha tudo
        queryUrl = `https://bible-api.com/${engBook}+${chapter}:1-300?translation=almeida`;
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto pb-48">
          ...
          {loadingEpigraph ? (
            ...
            <div className="max-w-3xl mx-auto mt-8 px-6 text-center animate-in fade-in zoom-in duration-500">
          ...
          <div className="p-6 max-w-3xl mx-auto space-y-4">
            {loading ? (
                ...
                verses.map(v => (
                    <div
                        key={v.number} 
                        className={`relative pl-8 pr-2 py-1 rounded transition-colors group cursor-pointer ${selectedVerse?.number === v.number ? 'bg-[#C5A059]/20' : 'hover:bg-gray-50 dark:hover:bg-gray-800'}`}
                        onClick={() => setSelectedVerse(v)}
                    >
                        <span className="absolute left-1 top-1 font-cinzel font-bold text-[#8B0000] dark:text-[#ff6b6b] text-xs opacity-60 select-none w-6 text-right">
                            {v.number}
                        </span>
                        <p 
                            className="font-cormorant leading-relaxed tracking-wide text-[#1a0f0f] dark:text-gray-200 text-justify"
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white dark:bg-[#121212] border-t border-[#C5A059] shadow-[0_-5px_20px_rgba(0,0,0,0.2)] pb-6 pt-4 px-4 z-[100] flex justify-between items-center safe-bottom">
         <button 
            ...
            disabled={chapter <= 1 && book === 'Gênesis'}
         ...
         <button 
            ...
            disabled={chapter >= totalChapters && book === 'Apocalipse'}
            className="flex flex-col items-center justify-center p-2 text-gray-600 dark:text-gray-300 hover:text-[#8B0000] disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
         >
