...
// ==========================================================================================
// COMPONENTE: PANORAMA BÍBLICO EBD - EDIÇÃO MAGNUM OPUS SUPREMA (v55.0)
...
// 8. VOLUME DE CÓDIGO: EXPANSÃO MASSIVA PARA > 1250 LINHAS (OBEDIÊNCIA AO ADMIN).
// ==========================================================================================
...
import { 
  ...
  BookCopy, FilePlus, Share2, Clipboard, Navigation, SearchX
} from 'lucide-react';
 ...
 * v55.0: Correção Crítica de Escopo e Refinamento de Interface Mobile Universal.
 */
export default function PanoramaView({ isAdmin, onShowToast, onBack, userProgress, onProgressUpdate }: PanoramaProps) {
  
  // --- ESTADOS DE CONTEÚDO E NAVEGAÇÃO DE PÁGINA ---
  // Livro Bíblico Alvo do Estudo
  const [book, setBook] = useState('Gênesis');
  // Capítulo Bíblico Alvo do Estudo
  const [chapter, setChapter] = useState(1);
  // Conteúdo Bruto recuperado do Banco de Dados
  const [content, setContent] = useState<EBDContent | null>(null);
  // Perfil de Leitura Ativo: Aluno (Leitura) ou Professor (Manual Docente)
  const [activeTab, setActiveTab] = useState<'student' | 'teacher'>('student');
  // Página Atual do Fragmento de Estudo
  const [currentPage, setCurrentPage] = useState(0);
  // Lista de Páginas formatadas após a exegese microscópica
  const [pages, setPages] = useState<string[]>([]);
  
  // --- ESTADOS DE GERAÇÃO (SISTEMA MAGNUM OPUS - RETENÇÃO 100%) ---
  // Sinaliza se a IA está processando o novo manuscrito
  const [isGenerating, setIsGenerating] = useState(false);
  // Cronômetro de tempo real para acompanhamento da auditoria
  const [generationTime, setGenerationTime] = useState(0);
  // Índice da mensagem de status atual para feedback visual
  const [currentStatusIndex, setCurrentStatusIndex] = useState(0);
  // Instruções adicionais do Administrador para guiar a IA
  const [customInstructions, setCustomInstructions] = useState('');
  // Controle de visibilidade do campo de comando especial
  const [showInstructions, setShowInstructions] = useState(false);
  // Densidade Teológica (Barra de Progresso Visual) de 0 a 100%
  const [theologicalDensity, setTheologicalDensity] = useState(0); 
  // Log de Auditoria em tempo real para o Administrador Supremo
  const [validationLog, setValidationLog] = useState<string[]>([]);
  // Fase atual do pipeline de geração e validação acadêmica
  const [validationPhase, setValidationPhase] = useState<'none' | 'structural' | 'theological' | 'final' | 'retention' | 'releasing'>('none');
  // Estatísticas detalhadas do manuscrito (Palavras, Caracteres, Páginas Estimadas)
  const [stats, setStats] = useState({ wordCount: 0, charCount: 0, estimatedPages: 0 });
  
  // --- REFS DE CONTROLE DE FLUXO E SEGURANÇA ---
  // Buffer para armazenar conteúdo gerado antes da persistência final
  const pendingContentBuffer = useRef<EBDContent | null>(null);
  // Sinalizador de processo ativo para o motor de cronômetro
  const generationActiveRef = useRef<boolean>(false);
  // Gatilho de aceleração da barra de progresso (Fim do Loop Infinito de Geração)
  const accelerationRef = useRef<boolean>(false);
  // Referência para o container de scroll principal para controle suave de UX
  const scrollContainerRef = useRef<HTMLDivElement>(null);

  // --- ESTADOS DE EDIÇÃO E REVISÃO MANUAL ---
  // Sinaliza se o modo de edição direta está ativo (Apenas Admin)
  const [isEditing, setIsEditing] = useState(false);
  // Valor temporário do manuscrito durante a edição
  const [editValue, setEditValue] = useState('');
  // CORREÇÃO CRÍTICA: Estado de salvamento movido para DENTRO do componente
  const [isSaving, setIsSaving] = useState(false); 

  // --- ESTADOS DE ÁUDIO (SINTETIZAÇÃO PREMIUM V55) ---
  // Controla se a aula está sendo narrada por voz neural
  const [isPlaying, setIsPlaying] = useState(false);
  // Exibição do painel de configurações de áudio
  const [showAudioSettings, setShowAudioSettings] = useState(false);
  // Lista de vozes sintetizadas disponíveis no sistema operacional
  const [voices, setVoices] = useState<SpeechSynthesisVoice[]>([]);
  // Identificador da voz selecionada para a narração
  const [selectedVoice, setSelectedVoice] = useState<string>('');
  // Velocidade de reprodução da aula (0.5x a 2x)
  const [playbackRate, setPlaybackRate] = useState(1);
  // Referência para o motor de voz (SpeechSynthesis)
  const speechRef = useRef<SpeechSynthesisUtterance | null>(null);

  // --- ESTADOS DE UX, RESPONSIVIDADE E SWIPE ---
  // Coordenada inicial do toque para detecção de swipe
  const [touchStart, setTouchStart] = useState<number | null>(null);
  // Coordenada final do toque para detecção de swipe
  const [touchEnd, setTouchEnd] = useState<number | null>(null);
  // Estado de scroll da tela para transformações no header
  const [scrolled, setScrolled] = useState(false);
  // Identificação de dispositivo mobile para otimização de layout
  const [isMobile, setIsMobile] = useState(false);
  // Distância mínima em pixels para validar um comando de swipe
  const minSwipeDistance = 60;

  // --- MENSAGENS DE STATUS DE GERAÇÃO (MAGNUM OPUS CALIBRATION) ---
  /** 
   * Lista de feedbacks dinâmicos durante o processamento de alta densidade.
   * Projetado para manter o usuário engajado durante a redação de 2400 palavras.
   */
  const loadingStatusMessages = [
    ...
    "Processando densidade teológica final v55...",
  ...
  // --- CONSTANTES DE ACESSO E ESTADO DE LEITURA ---
  // Chave identificadora única do capítulo atual no banco de dados
  const studyKey = generateChapterKey(book, chapter);
  // Verifica se o usuário atual já marcou este capítulo como concluído
  const isRead = userProgress?.ebd_read?.includes(studyKey);
  // Permissão de visualização baseada no perfil (Docente requer privilégios de Admin)
  const hasAccess = activeTab === 'student' || isAdmin;

  // --- CICLO DE VIDA, MONITORAMENTO E SINCRONIZAÇÃO ---
  
  /**
   * Monitoramento de viewport para ajuste dinâmico da interface mobile.
   */
  useEffect(() => {
  ...
  /**
   * Gatilho automático de carregamento de conteúdo ao alterar Livro ou Capítulo.
   */
  useEffect(() => { loadContent(); }, [book, chapter]);

  /**
   * Gerenciador de scroll para efeitos visuais no header (Glassmorphism dinâmico).
   */
  useEffect(() => {
    const handleScroll = () => {
  ...
  /**
   * Motor de Cronômetro e Pipeline de Auditoria Teológica.
   * Gerencia a barra de progresso simulada até a conclusão real do processamento da IA.
   */
  useEffect(() => {
    let interval: any;
    if (isGenerating) {
        ...
        interval = setInterval(() => {
            ...
            // Lógica de Densidade Teológica (Barra de Progresso Inteligente)
            setTheologicalDensity(prev => {
                // Se o conteúdo já foi recebido da IA, dispara a barra para 100% (Modo Turbo)
                if (accelerationRef.current) return Math.min(100, prev + 15); 
                // Senão, progride lentamente simulando o raciocínio teológico denso
                if (prev < 99) return prev + (100 / 280); 
                return 99;
            });

            // Rotatividade de mensagens informativas a cada 10 segundos
  ...
  /**
   * Carregamento de vozes sintetizadas com prioridade para perfis Google/Neural.
   */
  useEffect(() => {
    const loadVoices = () => {
        let ptVoices = window.speechSynthesis.getVoices().filter(v => v.lang.includes('pt'));
        // Priorizar vozes de alta qualidade (Google ou Neurais)
  ...
  /**
   * Reset de Áudio ao navegar entre fragmentos ou capítulos para evitar sobreposição.
   */
  useEffect(() => {
  ...
  // --- CARREGAMENTO E ANÁLISE QUANTITATIVA DE DADOS ---
  /**
   * Recupera o manuscrito do banco de dados e calcula métricas acadêmicas.
   */
  ...
  /**
   * Analisa a densidade do texto para auditoria do administrador.
   */
  const calculateStats = (text: string) => {
      ...
      // Meta acadêmica ADMA: ~450 palavras por página de alta densidade
      const estPages = Math.ceil(words / 450); 
      setStats({ wordCount: words, charCount: chars, estimatedPages: estPages });
  };

  /**
   * Efeito para processar a paginação sempre que o manuscrito ou a aba mudar.
   */
  useEffect(() => {
    if (content) {
  ...
  // --- ALGORITMO DE PAGINAÇÃO ACADÊMICA (ALTA PERFORMANCE) ---
  /**
   * Divide o manuscrito denso em fragmentos de leitura otimizados para dispositivos móveis.
   */
  const processAndPaginate = (html: string) => {
    if (!html || html === 'undefined') { setPages([]); return; }
    
    // Divisão baseada em quebras de página manuais (<hr>) ou marcadores de sistema
    let rawSegments = html.split(/<hr[^>]*>|__CONTINUATION_MARKER__/i)
                          .map(s => s.trim())
                          .filter(s => s.length > 50);

    // Proteção contra Páginas Gigantescas (Split automático por subtópicos se necessário)
    ...
    let currentBuffer = "";
    // Limite técnico de caracteres por página para manter fluidez de renderização
    ...
    for (let i = 0; i < rawSegments.length; i++) {
        ...
        if (!currentBuffer) {
            ...
            // Verifica se a união ultrapassa o limite visual
  ...
  // --- MOTOR DE SINTETIZAÇÃO VOCAL (TTS CHUNKING) ---
  /**
   * Converte o fragmento de estudo em áudio utilizando técnicas de chunking para evitar latência.
   */
  const speakText = () => {
    ...
    // Limpeza de caracteres especiais do Markdown para uma narração natural
  ...
  // --- RENDERIZADORES DE ESTÉTICA SUPREMA (v55.0) ---
  /**
   * Processa estilos de texto inline (Negrito e Itálico) com cores da marca.
   */
  ...
  /**
   * Renderizador Principal: Transforma Markdown teológico em componentes React estilizados.
   * Calibrado para leitura acadêmica confortável.
   */
    ...
    return (
        <div className="space-y-8 md:space-y-12 animate-in fade-in duration-1000">
            {lines.map((line, lineIdx) => {
                ...
                if (trimmed === '__CONTINUATION_MARKER__') {
                    return (
                        <div key={lineIdx} className="my-10 md:my-20 flex items-center justify-center select-none opacity-30">
                            ...
                            <span className="mx-6 text-[#C5A059] text-[10px] font-cinzel tracking-[0.5em] uppercase whitespace-nowrap">Continuidade do Manuscrito</span>
                            <div className="h-[1px] bg-[#C5A059] w-full"></div>
                        </div>
                    );
                }

                // Título Monumental da Apostila
                if (trimmed.toUpperCase().includes('PANORÂMA BÍBLICO') || trimmed.toUpperCase().includes('PANORAMA BÍBLICO')) {
                    return (
                        <div key={lineIdx} className="mb-12 md:mb-20 text-center border-b-[4px] border-[#8B0000] pb-6 pt-4">
                            <h1 className="font-cinzel font-bold text-2xl md:text-5xl text-[#8B0000] dark:text-[#ff6b6b] uppercase tracking-[0.2em] leading-tight">{trimmed}</h1>
                        </div>
                    );
                }

                // Cabeçalhos de Tópicos e Seções Especiais (Arqueologia/Tipologia)
                ...
                if (isHeader) {
                    ...
                    return (
                        <div key={lineIdx} className={`mt-14 mb-8 md:mt-24 md:mb-14 flex flex-col items-center gap-4 ${isUltra ? 'p-10 bg-black dark:bg-[#050505] rounded-[3rem] shadow-2xl border-t-[4px] border-[#C5A059]' : ''}`}>
                            <h3 className={`font-cinzel font-bold text-lg md:text-3xl uppercase tracking-[0.2em] text-center leading-relaxed ${isUltra ? 'text-[#C5A059]' : 'text-gray-900 dark:text-[#E0E0E0]'}`}>
                                {title}
                            </h3>
                ...
                // Tópicos Numerados com Microscopia Bíblica
                ...
                if (isListItem) {
                    ...
                    return (
                        <div key={lineIdx} className="mb-10 flex gap-4 md:gap-10 items-start pl-2 animate-in slide-in-from-left-6 duration-700">
                            <span className="font-cinzel font-bold text-3xl md:text-4xl text-[#C5A059] opacity-80 mt-1">{numberPart}</span>
                            <div className="flex-1 border-l-[3px] border-[#C5A059]/15 pl-6 md:pl-10 transition-colors hover:border-[#C5A059]/50">
                                <div className="font-cormorant text-xl md:text-2xl leading-relaxed text-gray-950 dark:text-gray-100 text-justify tracking-wide font-medium">{parseInlineStyles(textPart)}</div>
                            </div>
                        </div>
                    );
                }

                // Destaques de Curiosidades ou Perguntas Retóricas
                if (trimmed.toUpperCase().includes('CURIOSIDADE') || trimmed.toUpperCase().includes('ARQUEOLOGIA') || trimmed.endsWith('?')) {
                    return (
                        <div key={lineIdx} className="my-12 md:my-16 mx-2 font-cormorant text-xl md:text-2xl text-gray-800 dark:text-gray-200 italic bg-[#C5A059]/10 p-10 rounded-[3rem] border border-[#C5A059]/40 text-justify relative group">
                            ...
                            <div className="flex items-center gap-3 mb-4 text-[#C5A059]">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-[0.4em] font-montserrat">Auditoria Exegética v55</span>
                            </div>
                ...
                // Parágrafos Regulares de Explicação
                return (
                    <p key={lineIdx} className="font-cormorant text-xl md:text-2xl leading-loose text-gray-950 dark:text-gray-50 text-justify indent-12 md:indent-20 mb-10 tracking-wide font-medium">
  ...
  // ==========================================================================================
  // GERAÇÃO MAGNUM OPUS - PROTOCOLO MICHEL FELIX v55.0
  // ==========================================================================================
  /**
  ...
  const handleGenerate = async (mode: 'start' | 'continue') => {
    ...
    accelerationRef.current = false;
    setValidationLog(["🚀 Motor Exegético Michel Felix v55 Ativado", "📐 Configurando target: ~2400 palavras (Alta Densidade)"]);
    ...
    const cleanContext = currentText.replace(/__CONTINUATION_MARKER__/g, ' ').slice(-12000);

    // --- LÓGICA DE INTRODUÇÃO (DIRETRIZ FIEL DO USUÁRIO) ---
    ...
    // --- WRITING STYLE (PERSONA MICHEL FELIX - OBEDIÊNCIA 100%) ---
    const WRITING_STYLE = `
        ...
        4. DIDÁTICA DOS TEXTOS POLÊMICOS E DIFÍCEIS: Apresente o leque de interpretações históricas para conhecimento, mas OBRIGATORIAMENTE conclua defendendo a interpretação Ortodoxa, Assembleiana e Biblicamente coerente.
        ...
        2. DETALHES QUE FAZEM A DIFERENÇA: Traga costumes da época, geografia e contexto histórico para iluminar o texto.
        3. DENSIDADE: Extraia todo o suco do texto. Se houver uma lista de nomes, explique a relevância. 
        4. PROIBIDO TRANSCREVER O TEXTO BÍBLICO: O aluno já tem a Bíblia. NÃO escreva o versículo por extenso. Cite apenas a referência (Ex: "No versículo 1...", ou "Em Gn 47:1-6...") e vá direto para a EXPLICAÇÃO.

        --- IDIOMAS ORIGINAIS E ETIMOLOGIA (INDISPENSÁVEL) ---
        1. PALAVRAS-CHAVE: Cite os termos originais (Hebraico no AT / Grego no NT) transliterados quando relevante.
        ...
        3. TÓPICOS DO ESTUDO (Use Numeração 1., 2., 3...):
           ...
           (Explicação microscópica detalhada, sem transcrição do texto bíblico).

        4. SEÇÕES FINAIS OBRIGATÓRIAS (No final de todo estudo):
        ...
        3. Se for CONTINUAÇÃO, não repita o título nem a introdução, siga para o próximo versículo onde parou.
    `;
    
    const instructions = customInstructions ? `\nINSTRUÇÕES EXTRAS DO ADMIN: ${customInstructions}` : "";
    const continuationInstructions = `MODO CONTINUAÇÃO CRÍTICA: Você já iniciou a apostila. Continue EXATAMENTE de onde parou: "...${cleanContext.slice(-1500)}...". Continue a exegese microscópica em porções de 2-3 versículos. AO CHEGAR NO VERSO FINAL DO CAPÍTULO, GERE as seções de Tipologia e Arqueologia.`;
    ...
    try {
        ...
        // Limpeza de wrappers técnicos indesejados
        ...
        setValidationLog(prev => [...prev, `✅ Conteúdo recebido: ${cleanedResult.length} caracteres.`, "🔍 Validando obediência estrutural v55..."]);
        ...
        // PROTOCOLO DE LIBERAÇÃO IMEDIATA (ACELERAÇÃO DA RETENÇÃO)
        pendingContentBuffer.current = data;
        setValidationPhase('retention');
        accelerationRef.current = true; // Ativa o turbo na barra de progresso

        const checkFinal = setInterval(async () => {
            if (theologicalDensity >= 100) {
                clearInterval(checkFinal);
                
                // Persistência definitiva na infraestrutura de dados
                ...
                setValidationLog(prev => [...prev, "💎 Manuscrito Magnum Opus Finalizado!", "🔓 Manuscrito Liberado com Sucesso para todos os alunos."]);
  ...
  // --- RENDERIZAÇÃO DA INTERFACE SUPREMA v55.0 ---
  return (
    <div 
    ...
    >
        {/* HEADER DE ALTO PADRÃO ADMA SUPREME */}
        <header 
        ...
        >
            ...
            <div className="flex flex-col items-center">
                ...
                <div className="flex items-center gap-3 opacity-60 mt-2">
                    ...
                    <span className="text-[10px] uppercase tracking-[0.6em] font-montserrat font-bold">Magnum Opus v55</span>
                </div>
            </div>
        ...
        {/* PAINEL DE CONTROLE DE ÁUDIO SINTETIZADO PREMIUM */}
        <AnimatePresence>
            {showAudioSettings && (
                <motion.div 
                    ...
                    <div className="p-8 max-w-4xl mx-auto flex flex-col gap-8">
                        <div className="flex justify-between items-center border-b pb-6 dark:border-white/10">
                            <div className="flex flex-col">
                                <span className="font-cinzel font-black text-xs md:text-sm uppercase tracking-[0.4em] text-[#8B0000] dark:text-[#C5A059]">Narração Magistral</span>
                                <span className="text-[10px] text-gray-400 mt-1 uppercase tracking-widest flex items-center gap-2"><Volume2 className="w-3 h-3"/> Prof. Michel Felix v55</span>
                            </div>
                            <button 
                                onClick={togglePlay} 
                                className="bg-[#C5A059] text-black px-10 py-4 rounded-full font-black flex items-center gap-4 shadow-xl hover:scale-105 active:scale-95 transition-all"
                            >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="flex flex-col gap-3">
                                <label className="text-[10px] font-black uppercase text-gray-500 tracking-[0.3em] flex items-center gap-2"><Smartphone className="w-3 h-3"/> Perfil Vocal</label>
                                <select 
                                    className="w-full p-4 border-2 border-[#C5A059]/30 rounded-2xl dark:bg-gray-800 dark:text-white outline-none font-bold" 
                                    value={selectedVoice} 
                            ...
                            <div className="flex flex-col gap-3">
                                <label className="text-[10px] font-black uppercase text-gray-500 tracking-[0.3em] flex items-center gap-2"><Zap className="w-3 h-3"/> Velocidade do Ensino</label>
                                <div className="flex gap-4">
                                    {[0.8, 1, 1.25, 1.5].map(r => (
                                        <button 
                                            key={r} 
                                            onClick={() => setPlaybackRate(r)} 
                                            className={`flex-1 py-4 rounded-2xl border-2 transition-all font-black text-sm ${playbackRate === r ? 'bg-[#8B0000] text-white border-[#8B0000] shadow-lg' : 'bg-gray-50 dark:bg-gray-900 border-transparent text-gray-500'}`}
                                        >
                                            {r}x
                                        </button>
                                    ))}
        ...
        <div className="bg-white dark:bg-dark-card p-6 border-b border-[#C5A059]/30 flex gap-4 shadow-xl shrink-0 items-center">
             <div className="flex-1 relative group">
                 ...
                 <select 
                    ...
                    className="w-full pl-16 pr-6 py-5 border-2 border-[#C5A059]/20 rounded-3xl font-cinzel text-lg dark:bg-gray-800 dark:text-white outline-none appearance-none font-bold shadow-sm"
                >
             ...
             <div className="w-32 md:w-40 relative group">
                 ...
                 <input 
                    ...
                    className="w-full pl-16 pr-6 py-5 border-2 border-[#C5A059]/20 rounded-3xl font-cinzel text-lg dark:bg-gray-800 dark:text-white outline-none font-bold shadow-sm" 
                    min={1} 
                />
             </div>
        </div>

        {/* SELEÇÃO DE PERFIL ACADÊMICO (ALUNO / PROFESSOR) */}
        <nav className="flex bg-[#F5F5DC] dark:bg-black border-b border-[#C5A059]/40 shrink-0 sticky top-[92px] md:top-[128px] z-30 shadow-md">
            <button 
                ...
                className={`flex-1 py-6 font-cinzel font-black text-xs md:text-sm uppercase tracking-[0.5em] flex justify-center items-center gap-4 transition-all relative ${activeTab === 'student' ? 'bg-[#600018] text-white' : 'text-gray-500'}`}
            >
                <BookCheck className="w-6 h-6" /> Aluno
                {activeTab === 'student' && <motion.div layoutId="tab-v55" className="absolute bottom-0 left-0 w-full h-[3px] bg-[#C5A059] shadow-[0_0_15px_#C5A059]" />}
            </button>
            <button 
                onClick={() => setActiveTab('teacher')} 
                className={`flex-1 py-6 font-cinzel font-black text-xs md:text-sm uppercase tracking-[0.5em] flex justify-center items-center gap-4 transition-all relative ${activeTab === 'teacher' ? 'bg-[#600018] text-white' : 'text-gray-500'}`}
            >
                {isAdmin ? <ShieldCheck className="w-8 h-8 text-[#C5A059]" /> : <Lock className="w-6 h-6" />} Professor
                {activeTab === 'teacher' && <motion.div layoutId="tab-v55" className="absolute bottom-0 left-0 w-full h-[3px] bg-[#C5A059] shadow-[0_0_15px_#C5A059]" />}
            </button>
        </nav>

        {/* CONSTRUTOR MAGNUM OPUS (EXCLUSIVO ADMIN SUPREMO) */}
        {isAdmin && !isEditing && (
            <div className="bg-[#050505] text-[#C5A059] p-6 shadow-2xl sticky top-[185px] md:top-[220px] z-20 border-b-8 border-[#8B0000] animate-in slide-in-from-top-10">
                {isGenerating ? (
                    <div className="flex flex-col items-center gap-6 py-4">
                        ...
                        {/* Telemetria de Pipeline de Geração */}
                        ...
                        <div className="w-full bg-white/5 h-3 rounded-full mt-4 overflow-hidden border border-white/10 p-0.5">
                            <motion.div 
                                initial={{ width: 0 }} animate={{ width: `${theologicalDensity}%` }} 
                                className="bg-gradient-to-r from-[#8B0000] via-[#C5A059] to-[#8B0000] h-full rounded-full shadow-[0_0_25px_#C5A059]"
                            />
                        </div>
                        <div className="flex justify-between w-full text-[10px] font-black uppercase tracking-[0.8em] opacity-40">
                            <span className="flex items-center gap-2"><Binary className="w-3 h-3"/> Auditoria Teológica v55</span>
                    ...
                    <>
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center gap-6">
                                ...
                                <div className="flex flex-col">
                                    <span className="font-cinzel text-lg font-black tracking-[0.6em] uppercase text-white">CONSTRUTOR MAGNUM v55</span>
                            ...
                            <button 
                                ...
                                className="text-[10px] font-black uppercase tracking-[0.4em] bg-white/5 px-8 py-3 rounded-2xl border border-white/15 hover:bg-white/10 transition-all"
                            >
                                {showInstructions ? 'Ocultar' : 'Comando Especial'}
                            </button>
                        </div>
                            ...
                            {showInstructions && (
                                ...
                                >
                                    <textarea 
                                        ...
                                        placeholder="Ex: Focar na arqueologia dos babilônios..." 
                                        className="w-full p-6 text-lg text-black rounded-[2.5rem] font-montserrat shadow-inner bg-[#FDFBF7] font-bold" 
                                        rows={3} 
                                    />
                                </motion.div>
                            )}
                        ...
                        <div className="flex gap-4">
                            <button 
                                ...
                                className="flex-2 px-10 py-6 bg-[#8B0000] border-4 border-[#C5A059]/40 rounded-[2.5rem] text-[10px] font-black uppercase tracking-[0.4em] text-white hover:bg-white hover:text-black transition-all flex items-center justify-center gap-6 shadow-2xl active:scale-95 group"
                            >
                            ...
                            {pages.length > 0 && (
                                <button 
                                    ...
                                    className="px-6 py-6 bg-red-900/60 text-red-300 border-4 border-red-500/30 rounded-[2.5rem] hover:bg-red-600 hover:text-white transition-all shadow-2xl"
        ...
        >
            ...
            {isAdmin && stats.wordCount > 0 && (
                <div className="fixed top-40 left-6 z-50 bg-[#1a0f0f]/90 backdrop-blur-xl p-5 rounded-2xl border border-[#C5A059]/30 text-[#C5A059] shadow-2xl hidden lg:flex flex-col gap-2 animate-in slide-in-from-left-4">
                    <div className="flex items-center gap-2 border-b border-[#C5A059]/15 pb-2 mb-1"><AlignLeft className="w-3 h-3"/> <span className="font-cinzel text-[9px] uppercase font-bold tracking-widest">Auditoria v55</span></div>
            ...
            {!hasAccess ? (
                <div className="text-center py-64 opacity-50 dark:text-white animate-in zoom-in duration-1000">
                    <ShieldAlert className="w-56 h-56 mx-auto text-[#8B0000] drop-shadow-2xl mb-12" />
                     ...
                     <div className="flex justify-between items-center mb-10 border-b-2 pb-6 dark:border-white/10">
                        ...
                        <div className="flex gap-4">
                            ...
                            <button onClick={async () => {
                                ...
                                if (content.id) await db.entities.PanoramaBiblico.update(content.id, data);
                                await loadContent(); setIsEditing(false); onShowToast('Manuscrito Revisado!', 'success');
                                setIsSaving(false);
                            }} className="px-10 py-5 text-[10px] font-black bg-green-600 text-white rounded-full shadow-xl uppercase tracking-widest transition-all">
                                {isSaving ? <Loader2 className="animate-spin w-4 h-4"/> : 'Salvar Versão'}
                            </button>
                        </div>
                     </div>
                     <textarea 
                        ...
                        className="w-full h-[70vh] p-10 font-mono text-xl border-none rounded-[3rem] bg-gray-50 dark:bg-black dark:text-gray-300 resize-none shadow-inner focus:ring-12 focus:ring-[#C5A059]/20 transition-all" 
                    />
                 </div>
                ...
                <article className="bg-white dark:bg-dark-card shadow-2xl p-8 md:p-24 min-h-[100vh] border border-[#C5A059]/20 relative rounded-[5rem] animate-in fade-in duration-1000 select-text overflow-hidden">
                     ...
                     <div className="absolute bottom-12 right-16 flex items-center gap-8 select-none opacity-40 hover:opacity-100 transition-all group">
                        <div className="h-[1px] w-24 bg-[#C5A059] group-hover:w-48 transition-all"></div>
                         ...
                         <footer className="mt-48 text-center border-t-8 border-dotted border-[#C5A059]/40 pt-48 animate-in slide-in-from-bottom-16 duration-[1.5s] relative">
                             ...
                             <button 
                                onClick={async () => {
                                 ...
                                 onShowToast('Glória a Deus! Conhecimento Registrado.', 'success');
                             }} disabled={isRead} 
                            ...
                            >
                                 {isRead ? <CheckCircle className="w-14 h-14" /> : <GraduationCap className="w-14 h-14 group-hover:rotate-[360deg] transition-transform duration-[4s]" />}
                                 <span className="relative z-10 tracking-[0.25em] uppercase">{isRead ? 'ARQUIVADO' : 'CONCLUIR E PONTUAR'}</span>
                             </button>
                         </footer>
                     )}
                </article>
                ...
                <div className="text-center py-64 bg-white dark:bg-dark-card rounded-[4rem] border-8 border-dashed border-[#C5A059]/30 animate-in fade-in duration-[1.5s] shadow-2xl relative overflow-hidden group">
                    ...
                    <p className="font-montserrat text-sm text-gray-500 uppercase tracking-[1.4em] mb-32 font-black">Aguardando transcrição magistral.</p>
                    {isAdmin && (
                        <div className="max-w-2xl mx-auto p-16 bg-[#8B0000]/10 rounded-[4rem] border-4 border-dashed border-[#8B0000]/20 flex flex-col items-center shadow-lg">
                            <Library className="w-16 h-16 text-[#8B0000] mb-8 opacity-80 animate-bounce" />
                            <p className="text-sm font-black text-[#8B0000] uppercase tracking-[0.6em] text-center leading-loose font-montserrat">Administrador ADMA SUPREMO: <br/> Utilize o motor Magnum Opus v55 para realizar a exegese completa.</p>
                        </div>
                    )}
                </div>
            )}
        ...
        {/* NAVEGAÇÃO FLUTUANTE COMPACTA (UI CORRIGIDA v55 - PARA ANDROID E IOS) */}
        <AnimatePresence>
            {pages.length > 1 && hasAccess && !isEditing && (
                ...
                >
                    <div className="bg-[#050505]/95 dark:bg-dark-card/95 backdrop-blur-xl border border-[#C5A059]/50 p-3 rounded-3xl flex justify-between items-center shadow-[0_30px_100px_-20px_rgba(0,0,0,1)] pointer-events-auto overflow-hidden">
                        {/* Botão Anterior Otimizado */}
                        <button 
                            onClick={() => { setCurrentPage(Math.max(0, currentPage - 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); }} 
                            disabled={currentPage === 0} 
                            className="flex items-center gap-2 px-6 py-4 bg-[#8B0000] text-white rounded-2xl font-black text-[9px] md:text-[11px] uppercase tracking-widest disabled:opacity-20 transition-all active:scale-90"
                        >
                        ...
                        <div className="flex flex-col items-center px-6 md:px-12 flex-1">
                            <div className="flex items-baseline gap-2">
                                <span className="font-cinzel font-black text-[#C5A059] text-2xl md:text-3xl tracking-widest">{currentPage + 1}</span>
                                <span className="opacity-30 text-white font-bold text-xs">/ {pages.length}</span>
                            </div>
                            <div className="w-full max-w-[120px] md:max-w-[200px] bg-white/10 h-1 rounded-full mt-3 overflow-hidden">
                                <motion.div 
        ...
        {/* BARREIRA DE SEGURANÇA FINAL v55.0 */}
        <div className="h-40 shrink-0 select-none pointer-events-none opacity-0">ADMA SUPREME SECURITY LAYER v55.0</div>
    </div>
  );
}
