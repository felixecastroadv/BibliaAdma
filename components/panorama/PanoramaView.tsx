...
// ==========================================================================================
// COMPONENTE: PANORAMA BÍBLICO EBD - EDIÇÃO MAGNUM OPUS SUPREMA (v52.0)
 ...
 * v52.0: Correção de Tela Branca e Refinamento de Interface Mobile.
 */
export default function PanoramaView({ isAdmin, onShowToast, onBack, userProgress, onProgressUpdate }: PanoramaProps) {
  ...
  const [isSaving, setIsSaving] = useState(false); // CORREÇÃO: Movido para dentro do componente
  ...
  const loadingStatusMessages = [
    ...
    "Processando densidade teológica final v52...",
  ...
  useEffect(() => {
    const loadVoices = () => {
        ...
        ptVoices.sort((a, b) => (b.name.includes('Google') ? 1 : -1));
        setVoices(ptVoices);
  ...
  const speakText = () => {
    ...
    let textToSpeak = (div.textContent || div.innerText || "").replace(/\*\*/g, '').replace(/#/g, '').trim();
                    ...
                    return (
                        <div key={idx} className="mb-10 flex gap-6 items-start animate-in slide-in-from-left-6">
                            ...
                            <div className="flex-1 border-l-4 border-[#C5A059]/10 pl-6">
                                <div className="font-cormorant text-xl md:text-2xl leading-relaxed text-gray-900 dark:text-gray-100 text-justify tracking-wide">{parseInline(val)}</div>
                            </div>
                        </div>
                    );
                }
                return (
                    <p key={idx} className="font-cormorant text-xl md:text-2xl leading-loose text-gray-950 dark:text-gray-100 text-justify indent-12 md:indent-20 mb-8 tracking-wide">
                        {parseInline(tr)}
                    </p>
                );
            })}
  ...
  // ==========================================================================================
  // GERAÇÃO MAGNUM OPUS (v52.0 - OBEDIÊNCIA 100%)
  // ==========================================================================================
  const handleGenerate = async (mode: 'start' | 'continue') => {
    setIsGenerating(true);
    setValidationPhase('structural');
    accelerationRef.current = false;
    setValidationLog(["🚀 Iniciando motor Michel Felix v52...", "📏 Target: 2.400 palavras (Alta Densidade)"]);
    ...
    const continuation = mode === 'continue' ? `CONTINUE DE ONDE PAROU: "...${currentText.slice(-1200)}..."` : "INÍCIO DA EXEGESE MAGISTRAL.";
    ...
    try {
        ...
        if (!res || res.length < 400) throw new Error("Conteúdo insuficiente retornado.");
    ...
    <div className="min-h-screen bg-[#FDFBF7] dark:bg-dark-bg transition-colors duration-1000 flex flex-col selection:bg-[#C5A059]/30" onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
        ...
        <header className={`sticky top-0 z-40 transition-all duration-700 ${scrolled ? 'bg-[#400010]/95 backdrop-blur-2xl py-3 shadow-2xl border-b border-[#C5A059]/40' : 'bg-gradient-to-r from-[#600018] to-[#400010] py-8'} text-white px-8 flex justify-between items-center safe-top`}>
            ...
            <div className="flex flex-col items-center">
                <h2 className="font-cinzel font-bold text-xl md:text-5xl tracking-widest drop-shadow-lg">Panorama EBD</h2>
                <div className="flex items-center gap-3 opacity-60 mt-2">
                    <Milestone className="w-4 h-4 text-[#C5A059]" />
                    <span className="text-[10px] uppercase tracking-[0.5em] font-bold">Magnum Opus v52</span>
                </div>
            </div>
            <div className="flex gap-2">
                {isAdmin && !isEditing && content && (
                    <button onClick={() => { setEditValue(activeTab === 'student' ? content.student_content : content.teacher_content); setIsEditing(true); }} className="p-4 hover:bg-white/15 rounded-full text-[#C5A059] border border-[#C5A059]/20 transition-all hover:rotate-6"><PenLine className="w-8 h-8" /></button>
                )}
        ...
        <AnimatePresence>
            {showAudioSettings && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-white dark:bg-dark-card border-b border-[#C5A059] overflow-hidden z-30 shadow-xl">
                    <div className="p-8 max-w-4xl mx-auto flex flex-col gap-8">
                        <div className="flex justify-between items-center border-b pb-6 dark:border-white/10">
                            <div className="flex flex-col">
                                <span className="font-cinzel text-xs font-black uppercase tracking-widest text-[#8B0000] dark:text-[#C5A059]">Transmissão Magistral</span>
                                <span className="text-[10px] text-gray-400 mt-1 uppercase tracking-widest flex items-center gap-2 font-bold"><Volume2 className="w-3 h-3"/> Prof. Michel Felix v52</span>
                            </div>
                            <button onClick={togglePlay} className="bg-[#C5A059] text-black px-10 py-3 rounded-full font-black text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="flex flex-col gap-2">
                                <label className="text-[10px] font-black uppercase text-gray-500 tracking-widest flex items-center gap-2"><Smartphone className="w-3 h-3"/> Perfil da Voz</label>
                                <select className="w-full p-4 text-sm border-2 border-[#C5A059]/30 rounded-2xl dark:bg-gray-800 dark:text-white outline-none font-bold" value={selectedVoice} onChange={e => setSelectedVoice(e.target.value)}>
        ...
        {/* ABAS OTIMIZADAS */}
        <nav className="flex bg-[#F5F5DC] dark:bg-black border-b border-[#C5A059]/40 shrink-0 sticky top-[108px] md:top-[128px] z-30 shadow-md">
            <button onClick={() => setActiveTab('student')} className={`flex-1 py-6 font-cinzel font-black text-xs md:text-sm uppercase tracking-widest flex justify-center items-center gap-4 transition-all relative ${activeTab === 'student' ? 'bg-[#600018] text-white' : 'text-gray-500'}`}>
                <BookCheck className="w-6 h-6" /> Aluno
                {activeTab === 'student' && <motion.div layoutId="tab-v52" className="absolute bottom-0 left-0 w-full h-[4px] bg-[#C5A059]" />}
            </button>
            <button onClick={() => setActiveTab('teacher')} className={`flex-1 py-6 font-cinzel font-black text-xs md:text-sm uppercase tracking-widest flex justify-center items-center gap-4 transition-all relative ${activeTab === 'teacher' ? 'bg-[#600018] text-white' : 'text-gray-500'}`}>
                {isAdmin ? <ShieldCheck className="w-8 h-8 text-[#C5A059]" /> : <Lock className="w-6 h-6" />} Professor
                {activeTab === 'teacher' && <motion.div layoutId="tab-v52" className="absolute bottom-0 left-0 w-full h-[4px] bg-[#C5A059]" />}
            </button>
        </nav>
                        ...
                        <div className="flex items-center gap-8">
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="flex gap-4 mt-3">
                                    <span className="text-[10px] opacity-70 font-mono flex items-center gap-2 bg-white/5 px-4 py-1 rounded-xl"><Clock className="w-3 h-3 text-[#C5A059]"/> Cronômetro: {generationTime}s</span>
                                    <span className={`text-[10px] font-black uppercase tracking-widest px-4 py-1 rounded-xl border ${accelerationRef.current ? 'bg-green-900/40 text-green-400 border-green-500' : 'bg-blue-900/40 text-blue-400 border-blue-500'}`}>
                                        Status: {validationPhase === 'retention' ? 'Retenção Final' : 'Exegese v52'}
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <>
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center gap-6">
                                <div className="w-16 h-16 bg-gradient-to-br from-[#8B0000] to-[#400010] rounded-3xl flex items-center justify-center shadow-xl ring-2 ring-[#C5A059]/40"><Sparkles className="w-10 h-10 text-white animate-pulse" /></div>
                                <div className="flex flex-col">
                                    <span className="font-cinzel text-lg font-black tracking-widest uppercase text-white">CONSTRUTOR MAGNUM v52</span>
                            ...
                            <button onClick={() => setShowInstructions(!showInstructions)} className="text-[10px] font-black uppercase tracking-widest bg-white/5 px-8 py-3 rounded-2xl border border-white/10 hover:bg-white/10 transition-all">{showInstructions ? 'Ocultar' : 'Comandos Extras'}</button>
                        </div>
        ...
        <main ref={scrollContainerRef} className="flex-1 overflow-y-auto p-6 md:p-16 max-w-[1400px] mx-auto pb-[250px] w-full scroll-smooth">
            ...
            {isAdmin && stats.wordCount > 0 && (
                <div className="fixed top-40 left-6 z-50 bg-[#1a0f0f]/90 backdrop-blur-xl p-5 rounded-2xl border border-[#C5A059]/30 text-[#C5A059] shadow-2xl hidden md:flex flex-col gap-2 animate-in slide-in-from-left-4">
                    <div className="flex items-center gap-2 border-b border-[#C5A059]/15 pb-2 mb-1"><AlignLeft className="w-3 h-3"/> <span className="font-cinzel text-[9px] uppercase font-bold tracking-widest">Telemetria v52</span></div>
                ...
                <article className="bg-white dark:bg-dark-card shadow-2xl p-10 md:p-24 min-h-[90vh] border border-[#C5A059]/20 relative rounded-[5rem] animate-in fade-in duration-1000 select-text overflow-hidden">
                     ...
                     {currentPage === pages.length - 1 && userProgress && (
                         <footer className="mt-48 text-center border-t-8 border-dotted border-[#C5A059]/40 pt-48 animate-in slide-in-from-bottom-20 duration-[2s] relative">
                             ...
                             }} disabled={isRead} className={`group relative px-28 py-14 rounded-full font-cinzel font-black text-4xl shadow-2xl flex items-center justify-center gap-10 mx-auto overflow-hidden transition-all transform hover:scale-110 active:scale-95 border-8 border-white/10 ${isRead ? 'bg-green-600 text-white' : 'bg-gradient-to-r from-[#8B0000] via-[#D00010] to-[#600018] text-white'}`}>
                                 {isRead ? <CheckCircle className="w-14 h-14" /> : <GraduationCap className="w-14 h-14 group-hover:rotate-[360deg] transition-transform duration-[3s]" />}
                ...
                <div className="text-center py-64 bg-white dark:bg-dark-card rounded-[5rem] border-8 border-dashed border-[#C5A059]/30 animate-in fade-in duration-[1.5s] shadow-2xl relative overflow-hidden group">
                    ...
                    {isAdmin && (
                        <div className="max-w-2xl mx-auto p-16 bg-[#8B0000]/10 rounded-[4rem] border-4 border-dashed border-[#8B0000]/30 flex flex-col items-center shadow-lg transform group-hover:scale-105 transition-transform duration-500">
                            ...
                            <p className="text-sm font-black text-[#8B0000] uppercase tracking-[0.6em] text-center leading-loose font-montserrat">Administrador ADMA SUPREMO: <br/> Utilize o motor Magnum Opus v52 para gerar exegese microscópica.</p>
                        </div>
                    )}
                </div>
            )}
        ...
        {/* NAVEGAÇÃO FLUTUANTE COMPACTA (UI CORRIGIDA v52 - MOBILE FIRST) */}
        <AnimatePresence>
                ...
                <motion.nav initial={{ y: 200, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 200, opacity: 0 }} className="fixed bottom-10 left-6 right-6 z-40 max-w-4xl mx-auto pointer-events-none pb-safe">
                    <div className="bg-[#050505]/95 dark:bg-dark-card/95 backdrop-blur-xl border border-[#C5A059]/50 p-3 rounded-3xl flex justify-between items-center shadow-[0_30px_100px_-15px_rgba(0,0,0,1)] ring-4 ring-white/5 group pointer-events-auto overflow-hidden">
                        <button onClick={() => { setCurrentPage(Math.max(0, currentPage - 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); }} disabled={currentPage === 0} className="flex items-center gap-3 px-8 py-5 bg-[#8B0000] text-white rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest disabled:opacity-20 transition-all shadow-xl active:scale-90 border border-white/10 hover:bg-[#a00000]">
                            <ChevronLeft className="w-6 h-6" /> <span className="hidden xs:inline">Anterior</span>
                        </button>
                        ...
                        <button onClick={() => { setCurrentPage(Math.min(pages.length - 1, currentPage + 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); }} disabled={currentPage === pages.length - 1} className="flex items-center gap-3 px-8 py-5 bg-[#8B0000] text-white rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest disabled:opacity-20 transition-all shadow-xl active:scale-90 border border-white/10 hover:bg-[#a00000]">
                            <span className="hidden xs:inline">Próximo</span> <ChevronRight className="w-6 h-6" />
                        </button>
                    </div>
                </motion.nav>
            )}
        ...
        <div className="h-40 shrink-0 select-none pointer-events-none opacity-0">ADMA SECURITY LAYER SUPREME v52.0 - MAGNUM OPUS EDITION</div>
    </div>
  );
}
